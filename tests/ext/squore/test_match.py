...
def test_repr(sqmatch1: SquoreMatch) -> None:
    assert repr(sqmatch1) == (
        "SquoreMatch(tpmatch=Match(id='1-14', draw.name='Baum', matchnr=14, "
...
def test_match_constructor_with_config_value(match1: TPMatch) -> None:
    match = SquoreMatch(tpmatch=match1, config={"numberOfPointsToWinGame": 11})
    data = match.model_dump()
    assert data.get("numberOfPointsToWinGame") == 11


def test_match_constructor_invalid_config_value(match1: TPMatch) -> None:
    with pytest.raises(ValidationError, match="should be a valid integer"):
        _ = SquoreMatch(tpmatch=match1, config={"numberOfPointsToWinGame": "eleven"})  # type: ignore[typeddict-item]


def test_match_constructor_invalid_config_key(match1: TPMatch) -> None:
    with pytest.raises(ValidationError, match="type=extra_forbidden"):
        _ = SquoreMatch(tpmatch=match1, config={"invalidKey": True})  # type: ignore[typeddict-unknown-key]
